...
const int relayPin = D0;   // Ensure this pin is correct for your board
#define SS_PIN 15 //D8
#define RST_PIN 2 //D4
MFRC522 mfrc522(SS_PIN, RST_PIN);
String card_num;

const char* ssid = "Pixel Os";
const char* password = "00000000";
const char *serverName = "http://192.168.204.176/projek/absensi-asrama/api/sensor/save";
...
void setup() {
  ...
  digitalWrite(relayPin, HIGH);  // Ensure relay starts in off position
  ...
  if (mfrc522.PICC_IsNewCardPresent() && mfrc522.PICC_ReadCardSerial()) {
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      if (httpResponseCode == 200) {
        ...
        digitalWrite(relayPin, LOW);  // Turn on the relay (LOW is active)
        delay(5000); 
        digitalWrite(relayPin, HIGH);  // Turn off the relay (HIGH is inactive)
